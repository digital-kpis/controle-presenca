...
// ====== SUPABASE ======
// Instância única do Supabase — evita múltiplos clientes no mesmo contexto
function getSupabase(){
  const url=window.SUPABASE_URL, key=window.SUPABASE_KEY;
  if(!url||!key||url.includes("SEU_PROJETO")) return null;
  if(!window.__SB_INSTANCE__){
    window.__SB_INSTANCE__ = window.supabase.createClient(url, key, {
      realtime: { params: { eventsPerSecond: 10 } }
    });
  }
  return window.__SB_INSTANCE__;
}
...
function TelaConfig(){
  ...
  const salvar=async()=>{
    ...
    try{
      // Cria instância temporária só para testar as credenciais
      const sbTest = window.supabase.createClient(url.trim(), key.trim());
      const {error}=await sbTest.from("colaboradores").select("id").limit(1);
      // Se ok, salva e recria como instância global definitiva
      if(!error || error.code==="PGRST116"){
        await sbTest.removeAllChannels();
        window.__SB_INSTANCE__ = window.supabase.createClient(url.trim(), key.trim(), {
          realtime: { params: { eventsPerSecond: 10 } }
        });
      }
