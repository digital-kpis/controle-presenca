...
function ControlePresenca(){
  ...
  const isDSR=useCallback(statusId=>{
    ...
    // Só repete automaticamente se for DSR/Folga — nunca Férias, Falta, Atestado etc.
    return tipo && (tipo.id==="folga" || tipo.label?.toUpperCase().includes("DSR"));
  },[tiposStatus]);

  // Salva com retry automático (até 3 tentativas)
  const saveComRetry=useCallback(async(fn,rollback)=>{
    for(let i=0;i<3;i++){
      try{ const {error}=await fn(); if(!error) return true; console.warn(`Tentativa ${i+1} falhou:`,error); }
      catch(e){ console.warn(`Tentativa ${i+1} erro:`,e); }
  ...
  const cycleStatus=useCallback(async(colabId,dateIso)=>{
    ...
    if(isDSR(next)){
      // DSR: propaga para ±52 semanas no mesmo dia da semana
      ...
      setPresencas(prev=>{ const n={...prev}; datas.forEach(dt=>{ n[`${colabId}:${dt}`]=next; }); return n; });
      // Persiste em lotes de 20 para evitar timeout
      const LOTE=20;
      for(let i=0;i<datas.length;i+=LOTE){
        const lote=datas.slice(i,i+LOTE).map(dt=>({colab_id:colabId,data:dt,status:next}));
        await saveComRetry(()=>sb().from("presencas").upsert(lote,{onConflict:"colab_id,data"}));
      }
    } else if(isDSR(current)&&next==="vazio"){
      // Removendo DSR de um dia específico: só remove esse dia
      setPresencas(prev=>{ const n={...prev}; delete n[key]; return n; });
      await saveComRetry(
        ()=>sb().from("presencas").delete().eq("colab_id",colabId).eq("data",dateIso),
        ()=>setPresencas(prev=>({...prev,[key]:current}))
      );
    } else {
      // Status normal: só o dia clicado
      // Atualização otimista
      setPresencas(prev=>{ const n={...prev}; if(next==="vazio") delete n[key]; else n[key]=next; return n; });
      // Persiste com rollback se falhar
      const ok=await saveComRetry(
        ()=> next==="vazio"
          ? sb().from("presencas").delete().eq("colab_id",colabId).eq("data",dateIso)
          : sb().from("presencas").upsert({colab_id:colabId,data:dateIso,status:next},{onConflict:"colab_id,data"}),
        ()=>setPresencas(prev=>{ const n={...prev}; if(current==="vazio") delete n[key]; else n[key]=current; return n; })
      );
    }
  },[presencas,tiposStatus,isDSR,saveComRetry]);
