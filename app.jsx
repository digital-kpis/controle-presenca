...
const STATUS_SEED = [
  { id:"presente", label:"Presente",    short:"P",  color:"#5B8A72", bg:"#E8F0EA", fixo:true, peso:800 },
  { id:"falta",    label:"Falta",       short:"F",  color:"#C1503E", bg:"#F7E8E5", peso:800 },
  { id:"folga",    label:"Folga (DSR)", short:"D",  color:"#8A8478", bg:"#EFEDE8", peso:800 },
  { id:"ferias",   label:"Férias",      short:"FF", color:"#B08968", bg:"#F3EAE0", peso:800 },
  { id:"atestado", label:"Atestado",    short:"AT", color:"#7A6B8A", bg:"#ECE8F0", peso:800 },
];
const STATUS_VAZIO = { id:"vazio", label:"Não marcado", short:"", color:"#C9C3B8", bg:"#FFFFFF", peso:800 };

const PALETA_CORES = [
  // vivas / saturadas
  "#E8412C","#FF6B1A","#F2A100","#E8C400","#7CC93B",
  "#1FB35A","#00B8A9","#0EA5D9","#2E6BF2","#6A4CE0",
  "#B23CE0","#E83C8C","#FF4D6D","#00C2C2","#3DD97E",
  // tons neutros / terrosos (originais)
...
create table tipos_status (
  ...
  color text not null, bg text not null, peso int default 800,
...
-- Já tem o projeto criado? Rode só esta linha para atualizar:
alter table tipos_status add column if not exists peso int default 800;`
          )
        )
      ),
      h("div",{style:S.configStep},
        ...
        ...tiposStatus.map(s=>
          h("div",{key:s.id,style:S.legendItem},
            h("span",{style:{...S.legendSwatch,background:s.bg,borderColor:s.color}},
              h("span",{style:{color:s.color,fontSize:10,fontWeight:s.peso||800}},s.short)
            ),
      ...
      return h("td",{key,style:S.tdCell},
        h("button",{
          ...
          style:{
            ...
            fontWeight: marcado?(s.peso||900):700,
      ...
      h("div",{style:S.tiposList},
        ...tipos.map(t=>
          h("div",{key:t.id,style:S.tipoRow},
            h("span",{style:{...S.tipoSwatch,background:t.bg,borderColor:t.color,color:t.color,fontWeight:t.peso||800}},t.short),
...
function TipoForm({initial,onClose,onSave}){
  ...
  const [peso,setPeso]=useState(initial?.peso||800);
  const [error,setError]=useState("");
  const bg=hex=>`${hex}1A`;
  const submit=()=>{
    if(!label.trim()){setError("Dê um nome para o tipo.");return;}
    if(!short.trim()){setError("Defina uma sigla.");return;}
    onSave({id:initial?.id,label:label.trim(),short:short.trim().toUpperCase().slice(0,3),color,bg:bg(color),peso});
  };
  return h("div",{style:S.modalOverlay,onClick:onClose},
    h("div",{style:S.modalCard,onClick:e=>e.stopPropagation()},
      h("div",{style:S.modalHeader},
      ...
      h("div",{style:S.formGroup},h("label",{style:S.label},"Peso da fonte"),
        h("div",{style:S.pesoRow},
          ...PESOS_FONTE.map(p=>h("button",{key:p.id,onClick:()=>setPeso(p.id),
            style:{...S.chip,...(peso===p.id?S.chipActive:{}),fontWeight:p.id}
          },p.label))
        )
      ),
      h("div",{style:S.formGroup},h("label",{style:S.label},"Pré-visualização"),
        h("span",{style:{...S.tipoSwatch,background:bg(color),borderColor:color,color,fontWeight:peso,width:40,height:36,fontSize:13}},short||"—")
      ),
      error&&h("p",{style:S.errorText},error),
      h("div",{style:S.modalActions},
        h("button",{style:S.btnGhost,onClick:onClose},"Cancelar"),
