...
function ControlePresenca(){
  ...
  useEffect(()=>{
    ...
    const ch1=sb.channel("colabs-rt").on("postgres_changes",{event:"*",schema:"public",table:"colaboradores"},async()=>{
      const {data}=await sb.from("colaboradores").select("*").order("nome"); if(data) setColaboradores(data);
    }).subscribe();
    const ch2=sb.channel("presencas-rt").on("postgres_changes",{event:"*",schema:"public",table:"presencas"},(pl)=>{
      const {eventType,new:row,old}=pl;
      setPresencas(prev=>{const next={...prev}; if(eventType==="DELETE") delete next[`${old.colab_id}:${old.data}`]; else next[`${row.colab_id}:${row.data}`]=row.status; return next;});
    }).subscribe();
    const ch3=sb.channel("tipos-rt").on("postgres_changes",{event:"*",schema:"public",table:"tipos_status"},async()=>{
      const {data}=await sb.from("tipos_status").select("*").order("ordem"); if(data) setTiposStatus(data);
    }).subscribe();
    const ch4=sb.channel("bh-rt").on("postgres_changes",{event:"*",schema:"public",table:"banco_horas"},async()=>{
      const {data}=await sb.from("banco_horas").select("*").order("data",{ascending:false}); if(data) setBancoHoras(data);
    }).subscribe();
  ...
  // ---- PRESENÇA ----
  // Verifica se um tipo é DSR (folga semanal que se repete)
  const isDSR=useCallback(statusId=>{
    if(statusId==="vazio") return false;
    const tipo=tiposStatus.find(t=>t.id===statusId);
    return tipo && (tipo.id==="folga" || tipo.label?.toLowerCase().includes("dsr") || tipo.label?.toLowerCase().includes("folga"));
  },[tiposStatus]);
  ...
  const cycleStatus=useCallback(async(colabId,dateIso)=>{
    ...
    // Propaga DSR para todas as semanas (mesmo dia da semana, 52 semanas)
    const eraOuVaiSerDSR = isDSR(current) || isDSR(next);
    if(eraOuVaiSerDSR){
      ...
      // Atualização otimista de todas as semanas
      setPresencas(prev=>{
        const n={...prev};
        datas.forEach(dt=>{
          const k=`${colabId}:${dt}`;
          if(next==="vazio"||!isDSR(next)){
            // Se estava DSR e agora mudou, limpa só a data clicada; as outras permanecem
            if(dt===dateIso){ if(next==="vazio") delete n[k]; else n[k]=next; }
          } else {
            n[k]=next; // propaga DSR
          }
        });
        return n;
      });
      // Persiste no banco
      if(isDSR(next)){
        const rows=datas.map(dt=>({colab_id:colabId,data:dt,status:next}));
        await sb().from("presencas").upsert(rows);
      } else {
        // Removeu/trocou a DSR — limpa só o dia clicado
        if(next==="vazio") await sb().from("presencas").delete().eq("colab_id",colabId).eq("data",dateIso);
        else await sb().from("presencas").upsert({colab_id:colabId,data:dateIso,status:next});
      }
    } else {
      // Status normal — só o dia clicado
      setPresencas(prev=>{const n={...prev}; if(next==="vazio") delete n[key]; else n[key]=next; return n;});
      if(next==="vazio") await sb().from("presencas").delete().eq("colab_id",colabId).eq("data",dateIso);
      else await sb().from("presencas").upsert({colab_id:colabId,data:dateIso,status:next});
    }
  },[presencas,tiposStatus,isDSR]);
    ...
    h("header",{style:S.header},
      h("div",{style:S.headerTop},
        ...
        h("div",{style:{display:"flex",alignItems:"center",gap:8}},
          h("div",{style:{...S.onlineDot,background:online?"#5B8A72":"#C1503E"},title:online?"Online":"Offline"},
                ...
                gruposPorTurno.map(grupo=>
                  h("tbody",{key:grupo.turno.id},
                    ...
                    ...grupo.items.map((colab,i)=>rowColab(colab,i,weekDays,presencas,cycleStatus,
                      ...
                      ()=>setConfirmDeleteColab(colab),getStatusInfo,isDSR
                    ))
                  )
                ):
                h("tbody",null,
...
// ====== LINHA DA TABELA ======
function rowColab(colab,index,weekDays,presencas,onCycle,onEdit,onDelete,getStatusInfo,isDSRFn){
  return h("tr",{key:colab.id,style:index%2===1?{background:"#FBFAF7"}:undefined},
    ...
    ...weekDays.map(d=>{
      ...
      const ehDSR=isDSRFn&&isDSRFn(status);
      return h("td",{key,style:S.tdCell},
        h("button",{
          onClick:()=>onCycle(colab.id,dateIso),
          title: ehDSR?"DSR — repete toda semana":undefined,
          style:{
            ...S.cellBtn,
          ...
          ehDSR&&h("span",{style:{
            ...
            fontSize:7,lineHeight:1,opacity:0.85,
