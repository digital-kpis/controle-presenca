...
const GH = {
  ...
  async write(path, data, sha){
    ...
    const body={ message:`update ${path}`, content: btoa(unescape(encodeURIComponent(JSON.stringify(data,null,2)))), branch: this.branch };
    if(sha) body.sha=sha;
...
function ControlePresenca({ onDesconectar }){
  ...
  // Fila de presenças pendentes para save em lote
  const presencasPendentes = useRef({});
  ...
  const saveGitHub = useCallback(async(path, data, shaKey)=>{
    setSalvando(true);
    try{
  ...
  // ---- PRESENÇA: save em lote com debounce de 1.5s ----
  const flushPresencas = useCallback(async(novasPresencas)=>{
    clearTimeout(saveTimer.current);
    setPendentes(p=>p+1);
    saveTimer.current = setTimeout(async()=>{
      setSalvando(true);
      try{
        const sha = await GH.write("data/presencas.json", novasPresencas, shas.current.presencas||null);
        shas.current.presencas = sha;
        LC.set("presencas", novasPresencas);
        presencasPendentes.current = {};
        setPendentes(0);
      }catch(e){
        showToast("Erro ao salvar presenças: "+e.message, true);
        setPendentes(0);
      }finally{
        setSalvando(false);
      }
